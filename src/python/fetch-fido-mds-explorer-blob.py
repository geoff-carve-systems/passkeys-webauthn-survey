...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "Download all commits that touched mds.blob since the most recently "
...
def most_recent_download_date() -> date | None:
    ...
    Scans for files matching ``????????_mds.blob`` and returns the maximum date.
    Returns None if no matching files exist.
    """
    matches = list(OUTPUT_DIR.glob("????????_mds.blob"))
    if not matches:
        return None
    dates = []
    for p in matches:
...
async def fetch_blob_commits_since(
    ...
    """Fetch all commits that modified mds.blob on or after ``since`` (UTC).
    ...
    page = 1
    while True:
        params = {"path": "mds.blob", "since": since_str, "per_page": 100, "page": page}
        print(
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        if args.all:
            ...
            if since is None:
                ...
                sys.exit(1)
            print(f"[INFO] Fetching blob commits since {since.isoformat()} (UTC)", file=sys.stderr)
            try:
